              ...
              return new Promise(function (resolve, reject) {
                ...
                for (i = 0, l = result.rows.length; i < l; ++i) {
                  ...
                  result.rows[i] = {
                    ...
                    startTime: moment(cbm.startDateTime).format(constants.formats.time),
                    cloudsType: cbm.cloudsType,
                    threats: cbm.threatsBg,
                    observers: cbm.observers,
                    mto: cbm.mto,
                    startDate: moment(cbm.startDateTime).format(constants.formats.date),
                    observationDate: moment(cbm.observationDateTime).format(constants.formats.date),
                    observationTime: moment(cbm.observationDateTime).format(constants.formats.time),
                    monitoringCode: cbm.monitoringCode,
                    zone: cbm.zoneId,
                    rain: cbm.rainBg,
                    windSpeed: cbm.windSpeedBg,
                    endTime: moment(cbm.endDateTime).format(constants.formats.time),
                    visibility: cbm.visibility,
                    notes: (cbm.notes||'').replace(/[\n\r]+/g, ' '),
                    endDate: moment(cbm.endDateTime).format(constants.formats.date),

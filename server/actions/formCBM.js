              ...
              return new Promise(function (resolve, reject) {
                ...
                for (i = 0, l = result.rows.length; i < l; ++i) {
                  ...
                  result.rows[i] = {
                    ...
                    startTime: moment(cbm.startDateTime).format(api.config.smartbirds.time),
                    cloudsType: cbm.cloudsType,
                    threats: cbm.threatsBg,
                    observers: cbm.observers,
                    mto: cbm.mto,
                    startDate: moment(cbm.startDateTime).format(api.config.smartbirds.date),
                    observationDate: moment(cbm.observationDateTime).format(api.config.smartbirds.date),
                    observationTime: moment(cbm.observationDateTime).format(api.config.smartbirds.time),
                    monitoringCode: cbm.monitoringCode,
                    zone: cbm.zoneId,
                    rain: cbm.rainBg,
                    windSpeed: cbm.windSpeedBg,
                    endTime: moment(cbm.endDateTime).format(api.config.smartbirds.time),
                    visibility: cbm.visibility,
                    notes: (cbm.notes||'').replace(/[\n\r]+/g, ' '),
                    endDate: moment(cbm.endDateTime).format(api.config.smartbirds.date),
